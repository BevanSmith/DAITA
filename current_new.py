...
4. If the user asks to save something to a CSV:
   ...
   b. If the output is:
       - A **single value** (like a float), convert it to a Series before saving:
         value = df['column'].skew()
         pd.Series([value], index=['skewness']).to_csv('filename.csv')
         value  # return value
         Never write {{'skewness': value}} in a dictionary when saving — always use a list for values and `index` for names in the Series.

       - A **Series or DataFrame** (e.g., from `df.isnull().sum()` or `df.describe()`), save it directly:
         missing_data = df.isnull().sum()
         missing_data.to_csv('missing.csv')
         result = missing_data.to_string()
         result  # return the string to be displayed
